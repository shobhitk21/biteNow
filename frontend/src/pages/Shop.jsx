...
const Shop = () => {
    ...
    const handleShop = async () => {
        try {
            const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/item/get-by-shop/${shopId}`, { withCredentials: true })
            setItems(data.items)
            setShop(data.shop)
    ...
    return (
        <div className='min-h-screen bg-gray-50'>
            <button
                className='absolute top-4 left-4 z-20 flex items-center gap-2 bg-black/50 hover:bg-black/70 text-white px-3 py-2 rounded-full cursor-pointer shadow transition'
                onClick={() => navigate("/")}
            >
                <FaArrowLeft />
                <span>Back</span>
                ...
                <div className='relative w-full h-64 md:h-80 lg:h-96'>
                    ...
                    <div className='absolute inset-0 bg-gradient-to-b from-black/70 to-black/30 flex flex-col justify-center items-center text-center px-4'>
                        ...
                        <div className='flex items-center gap-[10px]'>
                            <FaLocationDot size={22} color='#EEEEEE' />
